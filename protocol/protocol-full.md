...
**Operational definition of the target condition.** The target condition is a **within-article reporting defect**: a defect in reporting that can be established from the article’s own content — its abstract, main text, tables, figures, and those supplementary materials that report results or describe the methods as actually applied (Section 2.3) — or, for its reference entries, by resolving the identifiers it gives against public bibliographic catalogues. Four constructs qualify, and are defined in the taxonomy below: statements within the article that cannot all be true as reported, whether against one another or against the definition of what they report, after allowing for rounding (see “Conditions not counted as a finding” below); an error determinable from a graphic alone; reporting that could distort the interpretation of the article’s own results; and a bibliographic claim about one of the article’s own reference entries that the catalogues contradict. Defects requiring judgement about the substance of an external source are outside the definition. A flag that adjudication confirms to be such a defect is a **confirmed finding**, and an article meets the target condition when it carries at least one.
...
1. **Internal reporting discrepancies.** Statements within the same article that cannot all be true as reported [@Francis2013], termed internal reporting discrepancies by Puljak et al. [@Puljak2020]. Most consist of two or more statements that contradict one another; some are established by a single statement that contradicts the definition of what it reports, such as a fractional number of patients or a proportion above 100%. Categories are numerical and non-numerical. A discrepancy between a figure and the text or a table belongs to this domain, not to the graphical domain, following Puljak et al. [@Puljak2020] and the internal/external distinction of Cooper et al. [@Cooper2001].
...
**Stage 1: benchmark development (124 articles).** We have assembled a benchmark of 124 published articles in which errors have already been reported in the peer-reviewed literature, together with 136 item-level error records drawn from those reports. Of the 125 articles and 140 records first assembled, one article not published in English was removed with its record, and three records were removed whose only source was a publisher correction notice: correction notices are not used as sources, because the version of record now distributed incorporates the correction, so the error a notice reports is no longer present in the PDF the pipeline examines. In the domains assigned by the source reports, the benchmark spans three of the four domains: 84 records of internal reporting discrepancies, 38 of spin, and 14 of graphical errors. Each record is reassessed against the definitions of Section 2.4, which may move it to another domain; at the time of writing, the first rater’s reassessment gives 88 internal reporting discrepancies, 37 spin, 10 graphical errors, and 1 other within-article concern. The distribution after adjudication by both raters will be reported with the stage 1 results. No benchmark record is a citation error, so recall in that domain cannot be developed against previously reported cases; citation errors confirmed during stage 1 will be added to the benchmark as records discovered in this study. Its sources include the abstract-versus-full-text assessment of Kamel and El-Sobky [@Kamel2023], the DAMASCENE within-report discrepancy catalogue [@Nowbar2014], the data-extraction discrepancies of Puljak et al. [@Puljak2020], and the 30 reports selected as containing spin in the abstract conclusion for the SPIIN trial [@Boutron2014]. The full list, with the source that reported each error, is released as described in Section 2.11 and identified in Appendix 4.

The pipeline will be run on these articles under every condition, and every flag will be adjudicated as in Section 2.4, including flags that do not correspond to a previously reported error. Each error already reported in the source literature will be checked against the version-of-record PDF by one rater (HS), and that judgement verified by a second rater; disagreements will be resolved through discussion and, if consensus cannot be reached, by a third rater (YK or KA). To make this check tractable, the locations and verbatim text of the values or statements a reported error involves, any arithmetic needed to assess it, and a candidate taxonomy category will be drafted with the assistance of a large language model (Claude, Anthropic). A drafted quotation is kept only if it matches the text of the PDF. The rater verifies every draft against the version-of-record PDF and alone makes the decision, assigns the category, and grades severity; no decision is drafted. Confirmed findings of this kind are expected, since most source reports examined only one domain or one part of the article. They will be added to the benchmark as additional item-level records, marked as discovered in this study rather than reported in the source literature, so that the benchmark used for the retention decision and for the configuration evaluation of Section 2.6 reflects everything confirmed to be present. The number of records added in this way will be reported, by domain. Candidates that are not confirmed will be categorized as rounding-compatible, rounding-sensitive/indeterminate, unit conversion, omission misread as a discrepancy, misread table structure, apparent discrepancy reconciled elsewhere in the article, reported error not present in the version of record, out-of-scope document, or other, and this classification will inform refinement of the prompt's exclusion clauses.

All 124 articles are used for development, with no subset held out, so performance measured on this benchmark is **not reported as an estimate of pipeline accuracy**. Two further properties would make such an estimate unsafe in any case: every article was selected because an error had already been found in it, so the prevalence of findings far exceeds that of the study population; and several source reports are public together with their labels, including the SPIIN supplement and the DAMASCENE dataset, so contamination of model training data cannot be excluded. Clean estimates come only from stage 2 and the main study.
...
A configuration that can be assembled from runs already executed for the full pipeline, such as a single frontier model at a single run or the open-weight model at three runs, is defined as that set of runs and is not executed again; its flags and token use are those of the runs it comprises. Only configurations using models outside the full pipeline, such as cheaper model tiers, are executed anew on the subsample.

Flags raised by a low-cost configuration that the full pipeline did not raise will be adjudicated by the procedure of Section 2.4, and any confirmed finding among them will be added to the reference standard before sensitivity is calculated for every configuration.
